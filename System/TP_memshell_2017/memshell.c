...
/*
 * Fonction de formattage de l'affichage de la m�moire libre
 */
...
/*
 * Fonction d'affichage de la m�moire occup�e
 */
void used()
{
	unsigned long i;
...
void help()
{
	...
	printf("1) init : initialisation ou r�initialisation de l'allocateur\n");
	printf("2) alloc <taille> : allocation d'un bloc m�moire\n");
	printf("\tLa taille peut �tre en d�cimal ou en h�xad�cimal (pr�fixe 0x)\n");
	printf("\tretour : identificateur de bloc et adresse de d�part de la zone\n");
	printf("3) free <identificateur> : lib�ration d'un bloc\n");
	printf("4) show : affichage la liste des blocs libres restants\n");	
	printf("\tsous la forme {adresse de d�part, taille}\n");
	printf("5) used : affichage de la liste des blocs occup�s\n");
	printf("\tsous la forme {identificateur, adresse de d�part, taille}\n");		
	printf("6) help : affichage de ce manuel\n");
	printf("7) exit : quitter le shell\n");
	
	printf("\nRemarques :\n");
	printf("1) Au lancement, le shell appelle mem_init\n");
	printf("2) Le shell supporte jusqu'� %d allocations entre deux initialisations\n", NB_MAX_ALLOC);			
}
...
void init()
{
	...
	printf("**** Mini-shell de test pour l'allocateur m�moire ****\n");
	printf("\tTapez help pour la liste des commandes\n");
	
	id_count = 1;
	
	/* initialisation de la memoire */
	printf("Initialisation de la m�moire (%d octets)...", HEAP_SIZE);
	mem_init();
	printf("OK\n");
		...
		switch(cmd) {
			...
			case ALLOC:
				...
				if (res == NULL) {
					...
					} else { /* pas de probleme, affichage de la zone allou�e */
