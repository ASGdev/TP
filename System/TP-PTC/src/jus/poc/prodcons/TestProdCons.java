...
public class TestProdCons extends Simulateur {
	...
	public static void main(String[] args){new TestProdCons(new Observateur()).start();
		ProdCons ProdCons = new ProdCons(3,3);
		int consultation =1;
		System.out.println("Voulez vous accédez à un journal d'évenements ? \n"
				+ "non (0)/ oui(1)");
		try {
		...
		while(consultation != 0){
			System.out.println("Que voulez vous consultez ? \n "
					...
					+ "4: La liste des messages consommés \n"
					+ "5: La liste des messages déposés \n"
					+ "6: La liste des messages retirés ");
			int choix = -1;
			try {
