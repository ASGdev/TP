...
public class TestProdCons extends Simulateur {
    ...
    protected void run() throws Exception{
        ...
        /* ressources */
        ress = new ProdCons(nbBuffer, display, observateur);
        ...
        for(int i = 0 ; i < nbProd ; i++){
             prod.add(new Producteur(this.observateur, tempsMoyenProduction, deviationTempsMoyenProduction,
            		 ress, rand.next(), nombreMoyenNbExemplaire, deviationNombreMoyenNbExemplaire));
             prod.get(prod.size()-1).start();
             observateur.newProducteur(prod.get(prod.size()-1));
        }
        
        /* on génère les consommateurs */
        for(int i = 0 ; i < nbCons ; i++){
            cons.add(new Consommateur(this.observateur, tempsMoyenConsommation, deviationTempsMoyenConsommation, ress));
            cons.get(cons.size()-1).start();
            observateur.newConsommateur(cons.get(cons.size()-1));
        }
