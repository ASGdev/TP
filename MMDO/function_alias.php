...
function affiche_wikipedia($city){	
	echo "<a href=\"https://fr.wikipedia.org/wiki/".$city."\">";
	echo $city;;
	echo "</a>";								
}

function geocodage($info_station){
	$lat=$info_station['latitude'];
	$lon=$info_station['longitude'];
$urlbase = "http://nominatim.openstreetmap.org/reverse";
$urlparams =  'format=json&lat='.$lat.'&lon='.$lon.'&zoom=18&addressdetails=1'; 
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, $urlbase . '?' . $urlparams);
